...
export default function SeatMap({ forceAdmin = false }: { forceAdmin?: boolean }) {
  ...
  return (
    <div className="w-full h-full min-h-[60vh] bg-gray-100 rounded-xl border border-gray-200 relative shadow-inner flex flex-col overflow-hidden">
      ...
      {/* 핵심 수정: TransformWrapper 제거, CSS 네이티브 스크롤로 교체 */}
      {/* overflow-auto + touch-action: pan-x pan-y 로 모바일 터치 스크롤 완전 지원 */}
      <div
      ...
      >
        <div className="inline-block p-4 min-w-full">
          <div
            className="grid gap-1.5 p-4 bg-white rounded-xl shadow-md border border-gray-100 mx-auto"
            style={{
              gridTemplateColumns: `repeat(${maxCol || 1}, minmax(40px, 40px))`,
              width: 'fit-content'
            }}
          >
            {grid.slice(1).map((row, rIdx) => (
              <React.Fragment key={`row-${rIdx + 1}`}>
                {row.slice(1).map((seat, cIdx) => {
                  if (!seat) return (
                    <div key={`empty-${rIdx + 1}-${cIdx + 1}`} className="w-10 h-10 bg-gray-50/50 rounded-sm" />
                  );
                  ...
                  return (
                    <button
                      ...
                      disabled={isDisabled}
                      className={cn(
                        'w-10 h-10 rounded-t-lg rounded-b-sm flex items-center justify-center text-[9px] font-bold transition-all duration-150 overflow-hidden whitespace-nowrap px-0.5 shadow-sm select-none',
                        seatClass,
                      ...
                      style={customStyle}
                      title={displayName ? `${displayName} (${seat.row}행 ${seat.col}열)` : `${seat.row}행 ${seat.col}열`}
                    >
          ...
          {selectedSeatInfo.participant ? (
            ...
            <div className="space-y-2 text-sm text-gray-700">
              <p className="font-medium text-gray-500">
                빈 좌석 ({grid.find(r => r.some(s => s?.id === selectedSeatInfo.seatId))?.find(s => s?.id === selectedSeatInfo.seatId)?.row}행 {grid.find(r => r.some(s => s?.id === selectedSeatInfo.seatId))?.find(s => s?.id === selectedSeatInfo.seatId)?.col}열)
              </p>
